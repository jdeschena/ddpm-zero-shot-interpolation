...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_t:torch.Tensor, t:torch.Tensor, **model_kwargs):
        ...
        y_uncond = torch.full(size=(x_t.shape[0],), fill_value=0, device=x_t.device)
        pred_eps_uncond = self.model(x_t, t, y_uncond)
        mean, logvar = torch.split(pred_eps_uncond, 3, dim=1)

        mean = self._predict_xt_prev_mean_from_eps(x_t, t.type(dtype=torch.long), mean)
    ...
    def p_sample(self, x_t:torch.Tensor, t:torch.Tensor, **model_kwargs) -> torch.Tensor:
        ...
        mean, std = self.p_mean_variance(x_t , t, **model_kwargs)
