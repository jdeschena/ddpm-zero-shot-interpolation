...
from diff_exp.utils import save_args_to_cfg, add_args, default_arguments as cli_defaults, parse_args, Timing, spectral_norm
from omegaconf import OmegaConf
from functools import partial


def add_sn(m, l):    
    if isinstance(m, (th.nn.Conv2d, th.nn.Linear)):
        spectral_norm(m, n_power_iterations=2, lipschitz=l, name="weight")
        spectral_norm(m, n_power_iterations=2, lipschitz=l, name="bias")
...
def main():
    ...
    apply_fn = lambda m: add_sn(m, args.lipschitz)
    classifier.apply(apply_fn)
    ...
    def _cond_fn(x, t, y, class_weight):
        ...
        with th.enable_grad():
            ...
            log_probs = log_probs * class_weight
            out = th.autograd.grad(log_probs.sum(), x_in)[0]
            return out
    
    class_weights = args.class_weights
    class_weights = th.tensor(class_weights).unsqueeze(0)
